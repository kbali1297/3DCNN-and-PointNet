...
class SimpleDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split):
        if (split == 'train'):
            self.sdfs, self.labels = generate_toy_data(4096)
        elif(split == 'val'):
            self.sdfs, self.labels = generate_toy_data(1024)
        
        
        #self.sdfs = np.expand_dims(self.sdfs,axis=0)
        #print(self.sdfs.shape)
        #self.labels = np.expand_dims(self.labels, axis=0)
        #print(self.labels.shape)
    def __getitem__(self, idx):
        sdf = np.expand_dims(self.sdfs[idx], 0)
        #label = np.expand_dims(self.labels[idx], 0)
        return sdf,self.labels[idx]
