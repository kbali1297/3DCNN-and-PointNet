...
class ShapeNetPoints(torch.utils.data.Dataset):
    ...
    def get_point_cloud(shapenet_id):
        ...
        # TODO Implement
        shape_id += ".obj"
        point_cloud = np.zeros((3,1024))
        with open(ShapeNetPoints.dataset_path / category_id /shape_id, "r") as fptr:
            count = 0
            for line in fptr:
                words = line.split()
                if(words[0]=='v'):
                    point_cloud[0,count] = float(words[1])
                    point_cloud[1,count] = float(words[2])
                    point_cloud[2,count] = float(words[3])
                    count+=1
