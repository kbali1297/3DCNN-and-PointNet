...
class TNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(self.bn3(self.conv3(x)))#x.shape:(b, 1024, num_points)
        x = torch.max(x,dim=2)[0] #x.shape:(b,1024) 
        x = self.relu(self.bn4(self.fc1(x))) 
        x = self.relu(self.bn5(self.fc2(x)))
        x = self.fc3(x)#x.shape : (b,k**2)
        # Adding the identity to constrain the feature transformation matrix to be close to orthogonal matrix
        identity = self.identity.repeat(b, 1) #shape : (b,k**2)
        x = x + identity
