...
class MLPConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride):
        ...
        self.model = nn.Sequential(nn.Conv3d(in_channels, out_channels, kernel_size,stride),
                                   nn.ReLU(),
...
class ThreeDeeCNN(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        self.backbone = nn.Sequential(MLPConv(1,48,6,2),
                                      MLPConv(48,160,5,2),
                                      MLPConv(160,512,3,2))
        ...
        self.full_predictor = nn.Sequential(nn.Linear(512*self.feature_cube_side**3,2048),
                                            nn.ReLU(),
                                            nn.Linear(2048,2048),
                                            nn.ReLU(),
        ...
        for d in range(backbone_features.shape[2]):
            for h in range(backbone_features.shape[3]):
                for w in range(backbone_features.shape[4]):
                    ...
                    partial_object_prediction = partial_predictor(backbone_features[:,:,d,h,w].view(batch_size,-1))
